...
def resize(cols=100, rows=100, img_weight=100, img_height=100, max_image_count=5000):
    ...
    files=[]
    files += glob.glob(f'input/**/*.jpg', recursive=True)
    files += glob.glob(f'input/**/*.jpeg', recursive=True)
    files += glob.glob(f'input/**/*.png', recursive=True)
    ...
    for file in files:
        ...
        bcsk = '\\'
        new_image.save(f'.output/{file.replace(bcsk, "/").split("/")[-1]}')
        count+=1
...
if __name__ == '__main__':
    ...
    max_image_count = 1000# maximum number of images used
